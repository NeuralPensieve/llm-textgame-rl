...
class MockTrie:
    eos_id = 0
    def __init__(self, start_token_id: int = 3):
        self.head = TrieNode(start_token_id)
    ...
    def insert(self, token_ids: List[int]):
        ...
        if MockTrie.eos_id == 0:
            MockTrie.eos_id = token_ids[-1]
        else:
            if MockTrie.eos_id != token_ids[-1]:
                raise ValueError("There is a mismatch between EOS token IDs. Each action sequence must end with the EOS token.")
            else:
                MockTrie.eos_id = token_ids[-1]  # Update the EOS ID to the last token in the sequence which should be the EOS token
    ...
    def update_head(self, new_head: int):
        ...
        for key in self.head.children.keys():
            if key == new_head:
                ...
                output.append(key)
                while key != MockTrie.eos_id and len(self.head.children) == 1:
...
def generate_mask(tries: List[MockTrie], vocab_size: int, data_type=torch.float32) -> torch.Tensor:
